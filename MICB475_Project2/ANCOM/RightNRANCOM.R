...
resp <- subset_samples(ps_right, response_patient=="nonresponsive" & Cohort_Short!="B")
set.seed(1)
out = ancombc(data = resp, tax_level = "Genus", 
...
tab_lfc = res$lfc
col_name = c("Taxon", "Intercept", "v3 - v2")
...
df_fig_v2 = df_lfc %>% 
  filter(abs(`v3 - v2`) > 1.5) %>%
  transmute(Taxon, 
            `v3 - v2` = round(`v3 - v2`, 2)) %>%
  pivot_longer(cols = `v3 - v2`,
...
p_v2 = df_fig_v2 %>%
  ...
  labs(x = NULL, y = NULL, title = "Right, nonresponsive") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
p_v2

ggsave("ANCOM/rightnonrespv2v3.jpg", p_v2)
