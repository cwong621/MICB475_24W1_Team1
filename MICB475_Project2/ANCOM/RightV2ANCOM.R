...
v2 <- subset_samples(ps_right, Visit==2 & Cohort_Short!="B")
set.seed(1)
...
tab_lfc = res$lfc
col_name = c("Taxon", "Intercept", "responsive - nonresponsive", "healthy - nonresponsive")
...
df_fig_v2 = df_lfc %>% 
  filter(abs(`responsive - nonresponsive`) > 1.5 | abs(`healthy - nonresponsive`) > 1.5) %>%
  transmute(Taxon, 
            `responsive - nonresponsive` = round(`responsive - nonresponsive`, 2),
            `healthy - nonresponsive` = round(`healthy - nonresponsive`, 2)) %>%
  pivot_longer(cols = `responsive - nonresponsive`:`healthy - nonresponsive`,
...
p_v2 = df_fig_v2 %>%
  ...
  labs(x = NULL, y = NULL, title = "Right cohort, Visit 2") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
p_v2

ggsave("ANCOM/rightv2.jpg", p_v2)
